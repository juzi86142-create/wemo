...
import type { DealerActionRecord } from "./dealer-fixtures";
...
export function DealerActionPanel({ kind, rows }: DealerActionPanelProps) {
  ...
  return <section className="dealer-workspace-panel" aria-labelledby={`dealer-${kind}-heading`}>
    ...
    <div className="dealer-filter-row" aria-label={`${kind} status filters`}>{statuses.map((item) => <button className={filter === item ? "is-active" : undefined} type="button" onClick={() => setFilter(item)} key={item}>{item}</button>)}</div>
