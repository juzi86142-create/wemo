...
export function CartView({ initialCart, preview }: { initialCart: Cart; preview: boolean }) {
  ...
  useEffect(() => {
    if (!preview) return;
    const storedCart = getPreviewCart();
    setCart(storedCart);
    notifyCartChange(storedCart);
  }, [preview]);
