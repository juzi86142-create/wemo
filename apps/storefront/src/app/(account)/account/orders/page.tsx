...
import { AccountOrderActions, getAccountOrders, getSession } from "../../../../features/account";
...
export default async function OrdersPage() {
  ...
  try {
    ...
    return <main className="account-page"><div className="account-heading"><p className="eyebrow">ORDER HISTORY</p><h1>Your orders.</h1><p>Every order, one place.</p></div>{response.items.length === 0 ? <StatusPanel kind="empty" title="No orders yet." description="Your next good idea could start with the collection." action={<Link className="button button-dark" href="/products">Explore products</Link>} /> : <div className="order-list">{response.items.map((order) => <article className="account-order-card" key={order.id}><Link className="order-row" href={"/account/orders/" + order.id}><span>{order.order_no}</span><strong>{order.status.replaceAll("_", " ")}</strong><span>{new Date(order.created_at).toLocaleDateString("en-US")}</span><b>↗</b></Link><AccountOrderActions order={order} /></article>)}</div>}</main>;
