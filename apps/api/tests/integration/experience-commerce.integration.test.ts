...
describeDatabase("experience and commerce integration", () => {
  const state = {
    app: null as Awaited<ReturnType<typeof createApiApp>> | null,
    server: null as FastifyInstance | null,
    database: null as Awaited<ReturnType<typeof setupIntegrationDatabase>> | null,
  };

  beforeAll(async () => {
    state.database = await setupIntegrationDatabase();
  ...
  afterAll(async () => {
    ...
    await teardownIntegrationDatabase(state.database);
  });
