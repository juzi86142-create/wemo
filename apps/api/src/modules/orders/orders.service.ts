...
export class OrdersService {
  constructor(
    ...
    private readonly experience: ExperienceRepository,
      ...
      actor?.audience === "staff"
        ...
        : actor?.audience === "dealer"
          ...
          : actor
            ...
            : parsed;
    return OrderListResponseSchema.parse(await this.stateStore.listOrders(scope));
  }

  async getOrder(id: unknown) {
    const parsedId = parseInput(OrderIdParamSchema, { id });
    const item = await this.stateStore.getOrderById(parsedId.id);
  ...
  async createOrder(body: unknown) {
    ...
    const existing = await this.stateStore.findOrderByRequestId(context.request_id);
    if (existing) {
      return OrderMutationResponseSchema.parse({
    ...
    const pricing = await this.stateStore.previewPricing({
      items: input.items,
    ...
    const item = await this.stateStore.createOrder({
      channel,
      user_id: userId,
    ...
    try {
      for (const line of orderItems) {
        const reservation = await this.stateStore.reserveInventory(
          {
      ...
      for (const reservationId of reserved) {
        try {
          await this.stateStore.releaseInventory(
            reservationId,
            context.request_id,
      ...
      await this.stateStore.transitionOrder(
        item.id,
        "cancelled",
        context.request_id,
        "库存预占失败",
  ...
  async updateStatus(id: unknown, body: unknown) {
    ...
    const context = this.requestContext.requireContext();
    const actor = this.authorization.requireActor();
    const before = await this.stateStore.getOrderById(parsedId.id);
    if (
      actor.audience !== "staff" &&
    ...
    const item = await this.stateStore.transitionOrder(
      parsedId.id,
      input.status,
      context.request_id,
      input.note,
