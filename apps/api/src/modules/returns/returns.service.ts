...
export class ReturnsService {
      ...
      actor?.audience === "staff"
        ...
        : actor?.audience === "dealer"
          ...
          : actor
            ...
            : parsed;
    return ReturnListResponseSchema.parse(await this.stateStore.listReturnRequests(scope));
  }
  ...
  async createReturn(body: unknown) {
    ...
    const item = await this.stateStore.createReturnRequest({
      ...input,
  ...
  async reviewReturn(id: unknown, body: unknown) {
    ...
    const before = await this.stateStore.getReturnRequestById(parsedId.id);
    const item = await this.stateStore.reviewReturnRequest(
      parsedId.id,
      context.request_id,
      input.decision,
      input.note,
