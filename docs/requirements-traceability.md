...
| 需求第 3 章：信息架构         | 唯一 React 应用的四个路由区域                                                                      | 页面路由与导航自动化巡检                                | in-progress |
| 需求第 4 章：前台官网         | `apps/storefront/src/features/public-site` 与对应 API                                              | P/FE/HOME/PLP/PDP/CNT/DLR/CT 全量验收                   | in-progress |
| 需求第 5 章：用户中心         | `apps/storefront/src/features/account` 与身份、购物车、订单 API                                    | 用户账户和 B2C 全流程                                   | in-progress |
...
| 需求第 13 章：UI/UX           | `packages/ui` 与 `apps/storefront`                                                                 | 组件、响应式、表单、WCAG 2.2 AA                         | in-progress |
...
| 区域       | ID                                                                                                                                                                                                                     | 责任模块                                   | 状态    |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ------- |
| 公开前台   | `P-001`, `P-010`, `P-011`, `P-012`, `P-013`, `P-014`, `P-020`, `P-021`, `P-030`, `P-031`, `P-032`, `P-040`, `P-041`, `P-042`, `P-043`, `P-050`, `P-051`, `P-060`, `P-070`, `P-071`, `P-072`, `P-080`, `P-081`, `P-082` | `apps/storefront/src/features/public-site` | in-progress |
| 用户中心   | `U-001`, `U-002`, `U-003`, `U-004`, `U-005`, `U-006`, `U-007`                                                                                                                                                          | `apps/storefront/src/features/account`     | in-progress |
...
| 需求组       | ID                                                                                                                                                       | 前端责任                                                                                                                  | 后端责任                                        | 状态    |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | ------- |
| 全站头部     | `FE-001`, `FE-002`, `FE-003`, `FE-004`, `FE-005`, `FE-006`                                                                                               | `apps/storefront/src/features/platform`、`apps/storefront/src/features/public-site`                                       | cms/search/auth/localization                    | in-progress |
| 首页         | `HOME-001`, `HOME-002`, `HOME-003`, `HOME-004`, `HOME-005`, `HOME-006`, `HOME-007`                                                                       | `apps/storefront/src/features/public-site`、`apps/storefront/src/features/admin`                                          | cms/media/analytics                             | in-progress |
| 商品列表     | `PLP-001`, `PLP-002`, `PLP-003`, `PLP-004`, `PLP-005`, `PLP-006`, `PLP-007`, `PLP-008`                                                                   | `apps/storefront/src/features/public-site`                                                                                | catalog/pricing/inventory/search/seo            | in-progress |
| 商品详情     | `PDP-001`, `PDP-002`, `PDP-003`, `PDP-004`, `PDP-005`, `PDP-006`, `PDP-007`, `PDP-008`, `PDP-009`, `PDP-010`, `PDP-011`, `PDP-012`, `PDP-013`, `PDP-014` | `apps/storefront/src/features/public-site`                                                                                | catalog/pricing/inventory/media/cms/dealers/seo | in-progress |
...
| 用户中心     | `USR-001`, `USR-002`, `USR-003`, `USR-004`, `USR-005`, `USR-006`, `USR-007`, `USR-008`, `USR-009`, `USR-010`                                             | `apps/storefront/src/features/account`                                                                                    | auth/identity/orders/returns/notifications      | in-progress |
...
| 搜索         | `SEA-001`, `SEA-002`, `SEA-003`, `SEA-004`, `SEA-005`, `SEA-006`, `SEA-007`                                                                              | `apps/storefront/src/features/public-site`                                                                                | search/analytics                                | in-progress |
| SEO          | `SEO-001`, `SEO-002`, `SEO-003`, `SEO-004`, `SEO-005`, `SEO-006`, `SEO-007`, `SEO-008`                                                                   | `apps/storefront/src/features/platform`、`apps/storefront/src/features/public-site`、`apps/storefront/src/features/admin` | seo/cms/catalog                                 | in-progress |
...
| --- | --- | --- | --- |
| `apps/api/src/runtime/commerce.state.ts`、`apps/api/src/modules/orders`、`apps/api/src/modules/quotes` | 订单/报价/库存幂等、状态机、快照、越权与企业边界 | `apps/api/tests/integration/experience-commerce.integration.test.ts`（使用 `TEST_DATABASE_URL` 执行）、`pnpm --filter @wemo/api test` | 生产 Prisma 路径已完成；集成套件在未配置测试数据库时明确跳过 |
...
| `apps/api/src/runtime/api-error.filter.ts`、`apps/api/src/modules/integrations`、`apps/api/src/modules/jobs`、`apps/api/src/modules/analytics`、`apps/api/src/modules/reports`、`apps/api/src/modules/settings` | 统一错误、request_id、审计、outbox/job/integration/settings 协作面 | `apps/api/tests/integration/api.integration.test.ts`、`apps/api/tests/integration/identity-dealers.integration.test.ts`、`apps/api/tests/integration/experience-commerce.integration.test.ts`、`pnpm check` | `pnpm check` 通过；3 个 HTTP 集成套件需用 `RUN_DATABASE_INTEGRATION=1 TEST_DATABASE_URL=...` 后执行 |
| `apps/api/src/modules/auth`、`apps/api/src/modules/identity`、`apps/api/src/modules/dealers` | actor、session、RBAC、company_id 边界、申请/企业/成员 | `apps/api/tests/integration/identity-dealers.integration.test.ts`、`pnpm --filter @wemo/api test` | Prisma 实体和业务路径已完成；集成测试需显式测试数据库 |
...
| 数据读取来源单一化 | 市场/语言/币种唯一来源=请求上下文（环境配置 .env，请求头显式覆盖）；catalog/search/seo 查询契约移除 market/locale 冗余参数；cart 上下文直读；analytics 事件事实取客户端上报；pricing preview currency 必填直读；经销商必填字段直读缺失即报错不伪造；审计 actor_id 真实化（可空，系统事件不再伪造 admin）；通知模板创建/更新契约分离 | `apps/api/src/runtime/request-context.store.ts`、`apps/api/src/modules/dealers/dealers.service.ts`、`apps/api/src/modules/audit/audit.service.ts` | typecheck 0；39 测试 + 1 skipped；真实 HTTP 冒烟：产品列表返回 env 市场语言对应文案，/search 200 契约形状正确 |
| 报价与订单正确性 | 报价 current_version 随评审/转单递增；转单写入订单行（真实 SKU/名称）；过期报价禁转单；价格优先级对齐 6.4（企业>价格表>等级>默认）；订单快照固化真实 SKU/名称并提交节点校验库存；订单号/申请号统一 generateBusinessNo；表单工单状态机收紧附录 D 六态 | `apps/api/src/modules/quotes/quotes.service.ts`、`apps/api/src/modules/pricing/pricing.service.ts`、`apps/api/src/modules/orders/orders.service.ts`、`apps/api/src/modules/forms/forms.service.ts` | typecheck 0；39 测试 + 1 skipped |
| 框架能力复用与去重 | terminus/throttler 落地；listResponse 统一；cms 模块重复表单实现移除；orders 模块死代码（reserveInventory/releaseInventory/updateOrderStatus）移除；seed 移入 tests 且非破坏性 | `apps/api/src/health/health.controller.ts`、`apps/api/src/runtime/request-context.store.ts`、`apps/api/tests/seed.ts` | pnpm check 全绿（212 ID 覆盖/架构/无物理外键/typecheck/39测试/构建/运行时健康） |

## 后端四轮补齐证据（2026-09-07 夜间）

| 模块切片 | 已实现范围 | 代码与测试入口 | 结果 |
| --- | --- | --- | --- |
| 折扣码 ADM-PR-004/5.2 | Redis 折扣码管理（后台 CRUD）；checkout 核销校验：停用/有效期/市场/商品范围/最低金额/使用次数；percent/fixed 折扣计算固化进订单快照与总额 | `apps/api/src/modules/pricing/pricing.service.ts`、`apps/api/src/modules/orders/orders.service.ts` | typecheck 0；39 测试 + 1 skipped；pnpm check 全绿 |
| Shipment 拆单 ORD-B2B-005/ADM-O-005 | 员工创建分批发货（承运商/运单号/行项数量）；超量拦截（按已发数量计算剩余）；发满整单 shipped 否则 partially_shipped；审计记录 | `apps/api/src/modules/orders/orders.service.ts`、`apps/api/src/modules/orders/orders.controller.ts` | typecheck 0；39 测试 + 1 skipped |
| Tier 主数据 6.4 | dealer_tiers 表迁移；后台 CRUD；seed 三档（Distributor/Wholesale/Retail Partner） | `packages/database/prisma/schema.prisma`、`apps/api/src/modules/dealers/dealers.service.ts` | 迁移已应用；seed 非破坏跳过已存在 |
| CMS 定时发布/预览/版本 ADM-C-003/004/005 | publish 支持 publish_at/archive_at；定时发布与下线在读取侧按时间生效（无调度器）；草稿预览令牌（Redis 7 天有效）；内容版本历史（更新前快照 Redis 列表） | `apps/api/src/modules/cms/cms.service.ts`、`apps/api/src/modules/cms/cms.controller.ts` | typecheck 0；39 测试 + 1 skipped |
...
| 真实邮件 19 章 | nodemailer 经 Mailpit SMTP 真实投递；业务事件通知 email 渠道即时发送；投递状态/消息 ID/失败原因回写 Redis；收件人邮箱按用户查库 | `apps/api/src/modules/notifications/notifications.service.ts` | 真实冒烟：admin 登录后 Mailpit 出现 account_mfa_challenge 邮件；checkout 后出现 order_confirmation |

## 后端五轮终检证据（2026-09-07 凌晨）

| 模块切片 | 已实现范围 | 代码与测试入口 | 结果 |
| --- | --- | --- | --- |
| 交易正确性 | 申请单号统一生成器；阶梯价按采购数量选档（9.2）；PO 编号与结算方式入单快照（ORD-B2B-001/002）；折扣码限定用户（ADM-PR-004）；退货行项校验（USR-007）；历史复购 Reorder（ORD-B2B-007）；经销商接受报价（QTE-004） | `apps/api/src/modules/pricing/pricing.service.ts`、`apps/api/src/modules/orders/orders.service.ts`、`apps/api/src/modules/returns/returns.service.ts`、`apps/api/src/modules/quotes/quotes.service.ts` | typecheck 0；39 测试 + 1 skipped |
| 购物车与库存 | 游客购物车按本地标识复用（5.2）；库存盘点端点（7.6）；预占超时读取侧释放（9.3）；B2C 与经销商市场开关生效（5.2/ACC-005） | `apps/api/src/modules/cart/cart.service.ts`、`apps/api/src/modules/inventory/inventory.service.ts`、`apps/api/src/modules/orders/orders.service.ts` | typecheck 0；39 测试 + 1 skipped |
| 平台能力 | sitemap 真实生成（SEO-004）；语言市场管理路由（12.1）；表单定义管理（CT-001）；审计覆盖商品/内容/设置/媒体/表单写操作（2.2）；服务端埋点 purchase/dealer_apply_submit/request_quote/contact_submit（18.1）；webhook HMAC 验签密钥化 + 支付回调联动订单状态（15.1/5.2） | `apps/api/src/modules/seo/seo.service.ts`、`apps/api/src/modules/localization/localization.controller.ts`、`apps/api/src/modules/forms/forms.service.ts`、`apps/api/src/modules/integrations/integrations.service.ts`、`apps/api/src/modules/analytics/analytics.service.ts` | 真实冒烟：sitemap 返回真实条目；语言创建 200；伪造 webhook 签名 401 |
...
| 报表聚合 | 七类报表真实聚合：订单/报价/申请/表单/库存/成员/媒体/搜索事件来自 PostgreSQL 与 Redis（18.2） | `apps/api/src/modules/reports/reports.service.ts` | 真实冒烟：dashboard 返回 54 单真实数据 |
| 产品列表与后台 | PLP 年龄/场景/技能筛选与六种排序（PLP-002/003）；数据请求后台工单处理（7.9）；经销商成员邀请一次性令牌（6.7） | `apps/api/src/modules/catalog/catalog.service.ts`、`apps/api/src/modules/identity/identity.service.ts`、`apps/api/src/modules/dealers/dealers.service.ts` | typecheck 0；39 测试 + 1 skipped |

## 后端二轮核对补齐证据（2026-09-07）

| 模块切片 | 已实现范围 | 代码与测试入口 | 结果 |
| --- | --- | --- | --- |
| 认证与授权 | 服务端会话解析替代客户端身份头（`Authorization: Bearer`）；scrypt 密码哈希；随机 32 字节会话令牌；邮箱验证令牌化（Redis）；修改密码与退出其他设备；角色权限落库（roles.permissions Json，迁移 `20260907014206_add_role_permissions`）；RBAC 模块:动作粒度；限流复用 @nestjs/throttler（登录/表单 10 次/分，搜索 60 次/分，全局 100 次/分）；错误结构统一（登录失败 401 带 request_id） | `apps/api/src/modules/auth/auth.service.ts`、`apps/api/src/runtime/request-context.store.ts`、`apps/api/src/http/request-id.interceptor.ts`、`apps/api/tests/integration/api.integration.test.ts` | typecheck 0；Vitest 39 passed + 1 skipped；真实 HTTP 冒烟：无令牌/伪造 x-wemo-actor → 401，admin/dealer 令牌 → 200/200 |
...
| 价格有效期 | previewPricing 仅匹配 valid_from/valid_to 有效期内的价格记录，未设置视为永久有效 | `apps/api/src/modules/pricing/pricing.service.ts` | typecheck 0；39 测试通过 |
| 框架能力复用 | 健康检查改用 @nestjs/terminus（PG+Redis 真实探活）；限流改用 @nestjs/throttler；listResponse 三处本地复制统一到 runtime 公共工具；bootstrap 复用 configureApplication；seed 脚本移入 tests 且非破坏性（按自然键跳过已存在，绝不删数据） | `apps/api/src/health/health.controller.ts`、`apps/api/src/health/health.service.ts`、`apps/api/src/app.module.ts`、`apps/api/tests/seed.ts` | typecheck 0；/api/v1/health 返回 database/redis up；seed 两次运行零删除 |
| 工具函数去重 | listResponse/nowIso/redis-hash/pagination 公共工具统一使用，模块内不再各自实现 | `apps/api/src/runtime/list-response.ts`、`packages/contracts/src/common/index.ts`、各模块 service | typecheck 0 |

## 本轮前端 B2C 结算证据（2026-09-08）

| 模块切片 | 已实现范围 | 代码与验证入口 | 状态与边界 |
| --- | --- | --- | --- |
| Storefront checkout | 游客联系人/收货地址、登录用户资料与地址预填边界、优惠码/备注字段、字段/API 错误展示、提交中状态；输入和响应分别经 `CheckoutCreateSchema`、`OrderMutationResponseSchema` 校验 | `apps/storefront/src/app/(public)/checkout/page.tsx`、`apps/storefront/src/features/commerce/checkout-form.tsx`、`apps/storefront/src/features/commerce/checkout-adapter.ts`、`checkout-validation.test.ts`、`docs/superpowers/verification/2026-09-08-b2c-checkout.md` | `in-progress`：contract mock checkout 已完成；真实 API checkout、真实订单持久化和支付相关验收仍待后端启动 |
...
| 本地账号登录与角色入口 | 提供 Customer、Dealer、Administrator 三个固定演示账号；本地校验邮箱和密码，经 `localStorage` 保持会话；按角色进入 `/account`、`/dealer`、`/admin`，角色不匹配与退出后均阻止展示受保护工作区 | storefront typecheck 通过；29 个测试文件、61 个测试通过；production build 通过；真实浏览器覆盖错误密码、三角色登录、角色串用、刷新保持和退出 | `demo-done`：仅用于无后端演示，不构成真实认证、服务端授权、MFA 或安全验收 |
