...
| M3 身份与用户中心 | 注册、验证、会话、地址、收藏、订单/售后入口、订阅、安全 | storefront/account + api | 用户全流程、隐私同意、账户删除申请通过验收 |
| M4 经销商全流程 | 申请/审核/补件/激活、企业隔离、目录、价格、快速下单、报价、团队 | storefront/dealer + storefront/admin + api | 申请到经销商登录及企业内业务闭环可运行 |
| M5 B2C/B2B 交易 | 购物车、结算、价格、库存预占、订单、付款、发货、退款、退货 | storefront + api | B2C 开关两种模式和 B2B 审核型订单均通过端到端测试 |
| M6 运营后台 | Dashboard、商品、价格、库存、订单、经销商、用户、内容、表单、SEO、设置、权限、审计 | storefront/admin + api | 附录 B 的 A-001 至 A-016 全部可用且按权限隔离 |
| M7 集成与运营数据 | 邮件、搜索、对象存储、ERP/WMS、支付、税务、物流、CRM、分析、报表 | api/admin | Webhook 签名/幂等、失败重试、任务追踪和核心事件验证通过 |
...
- 对应：`storefront/shell`、`storefront/localization-seo`、`ui/navigation`、`api/localization`、`api/settings`。
...
- 对应：`storefront/features/public-site`、`storefront/features/admin`、`api/catalog`、`api/media`。
...
- 对应：`storefront/features/public-site`、`storefront/features/admin`、`api/cms`、`api/seo`。
...
- 对应：`storefront/features/public-site`、`storefront/features/admin`、`api/search`、`api/analytics`。
...
- 对应：`storefront/features/account`、`storefront/features/dealer`、`storefront/features/admin`、`api/auth`、`api/identity`。
...
- 对应：`storefront/features/public-site`、`storefront/features/dealer`、`storefront/features/admin`、`api/dealers`。
...
- 对应：`api/pricing`、`api/inventory`、`storefront/features/admin`。
...
- 对应：`storefront/features/account`、`storefront/features/dealer`、`storefront/features/admin`、`api/cart`、`api/orders`、`api/payments`、`api/returns`。

### 9. 报价

- 经销商从商品或购物车发起请求；平台维护含运税、条款、交期和有效期的版本化报价。
- Accepted 后按报价快照转单；拒绝/过期不可直接转单；全状态通知和审计。
- 对应：`storefront/features/dealer`、`storefront/features/admin`、`api/quotes`。
...
- 对应：`api/forms`、`api/notifications`、`api/media`、`storefront/features/admin`、`storefront/features/dealer`。
...
- 对应：`api/integrations`、`api/jobs`、`api/settings`、`storefront/features/admin`。
